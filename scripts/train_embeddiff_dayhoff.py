...
# scripts/train_embeddiff_dayhoff.py
...
EMBED_PATH = os.path.join(ROOT, "embeddings", "dayhoff_embeddings.npy")
FASTA_PATH = os.path.join(ROOT, "data", "curated_thioredoxin_reductase.fasta")
CHECKPOINT_DIR = os.path.join(ROOT, "checkpoints")
FIGURE_PATH = os.path.join(ROOT, "figures", "fig2b_loss_dayhoff")
...
# === Hyperparameters ===
embedding_dim = 1280   # Dayhoff hidden_size is 1280
...
print(f"[INFO] Dayhoff embedding shape: {x.shape}")

# Normalize Dayhoff latents for diffusion (scale to reasonable range)
mu = x.mean(axis=0, keepdims=True)
sigma = x.std(axis=0, keepdims=True) + 1e-6
np.savez(os.path.join(ROOT, "embeddings", "dayhoff_stats.npz"), mu=mu, sigma=sigma)

# Scale to [-1, 1] range for better diffusion training
x = (x - mu) / sigma
x = np.tanh(x * 0.5)  # Scale to [-1, 1] range
print(f"[INFO] Applied scaled normalization to Dayhoff embeddings (range: [{x.min():.3f}, {x.max():.3f}]).")
...
best_model_path = os.path.join(CHECKPOINT_DIR, "best_embeddiff_mlp_dayhoff.pth")
...
for epoch in range(epochs):
    ...
    print(f"[Dayhoff] Epoch {epoch+1}/{epochs} | Train Loss: {train_losses[-1]:.4f} | Val Loss: {avg_val_loss:.4f}")
...
print(f"[Dayhoff] 📊 Final Test Loss: {test_loss:.4f}")

# === Save loss curves ===
os.makedirs(os.path.join(ROOT, "figures"), exist_ok=True)
np.save(os.path.join(ROOT, "figures", "train_losses_dayhoff.npy"), np.array(train_losses))
np.save(os.path.join(ROOT, "figures", "val_losses_dayhoff.npy"), np.array(val_losses))
...
plt.plot([], [], label="Dayhoff", alpha = 0.0)
...
print(f"✅ Training complete. Best Dayhoff model saved to {best_model_path}. Loss curves plotted. Test loss reported.")
