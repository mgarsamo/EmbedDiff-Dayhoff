# embeddings/sample_embeddings.py
import sys, os
...
OUTPUT_PATH = os.path.join(ROOT, "embeddings", "sampled_embeddings.npy")
FIGURE_PATH = os.path.join(ROOT, "figures", "fig3a_generated_tsne")
TSNE_COORDS_PATH = os.path.join(ROOT, "embeddings", "tsne_coords.npy")
TSNE_LABELS_PATH = os.path.join(ROOT, "embeddings", "tsne_labels.npy")

# === Parameters ===
embedding_dim = 1280
samples_per_class = 80
noise_std = 0.03  # <-- Try adjusting this (e.g., 0.02–0.1) for mild noise
...
print(f"🎯 Sampling {num_samples} real embeddings with mild noise (std={noise_std})...")
...
print(f"✅ Saved t-SNE coordinates to: {TSNE_COORDS_PATH}")
print(f"✅ Saved t-SNE labels to: {TSNE_LABELS_PATH}")
...
plt.legend(title="Protein Class", prop=font_props, title_fontproperties=font_props, frameon=True, loc='upper left')
plt.tight_layout()
