...
FIGURE_PATH = os.path.join(ROOT, "figures", "fig2b_loss")
...
x_train, x_temp, y_train, y_temp = train_test_split(x, one_hot, test_size=0.2, stratify=labels, random_state=42)
x_val, x_test, y_val, y_test = train_test_split(x_temp, y_temp, test_size=0.5, random_state=42)

def make_loader(x, y):
    data = np.hstack([x, y])
    return DataLoader(TensorDataset(torch.tensor(data, dtype=torch.float32)), batch_size=batch_size, shuffle=True)
...
class ImprovedConditionalMLP(MLPNoisePredictor):
    def __init__(self, dim, cond_dim):
        ...
        self.net = torch.nn.Sequential(
            torch.nn.Linear(dim + cond_dim + 1, 512),
...
best_model_path = os.path.join(CHECKPOINT_DIR, "best_embeddiff_mlp.pth")
...
for epoch in range(epochs):
    ...
    print(f"Epoch {epoch+1}/{epochs} | Train Loss: {train_losses[-1]:.4f} | Val Loss: {avg_val_loss:.4f}")
...
np.save(os.path.join(ROOT, "figures", "train_losses.npy"), np.array(train_losses))
np.save(os.path.join(ROOT, "figures", "val_losses.npy"), np.array(val_losses))
...
plt.tight_layout()
plt.savefig(FIGURE_PATH + "_train_val.png", dpi=300)
plt.savefig(FIGURE_PATH + "_train_val.svg")
plt.close()

print("✅ Training complete. Best model saved. Loss curves plotted. Test loss reported.")
