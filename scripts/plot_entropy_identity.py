...
df["status"] = df.apply(
    lambda row: "accepted" if row["entropy"] >= ENTROPY_THRESHOLD and 
                              MIN_IDENTITY <= row["identity"] <= MAX_IDENTITY else "filtered_out", axis=1)
...
# plt.title("Entropy vs Identity of Decoded Sequences", weight="bold")
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig("figures/fig5c_entropy_scatter.png", dpi=300)
plt.close()
