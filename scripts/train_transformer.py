...
# === Load dataset ===
data = torch.load("data/decoder_dataset.pt")
token_embeddings = data["token_embeddings"]   # (B, L, 1280)
sequences = data["sequences"]                 # (B, L)
esm_logits = data["esm_logits"]               # (B, L, V)

# === Dataset split (80/10/10) ===
dataset = TensorDataset(token_embeddings, sequences, esm_logits)
...
for epoch in range(EPOCHS):
    ...
    for token_embed, y_seq, esm_log in train_loader:
    ...
    val_loss = 0
    with torch.no_grad():
        for token_embed, y_seq, esm_log in val_loader:
            token_embed, y_seq = token_embed.to(device), y_seq.to(device)
    ...
    print(f"[Epoch {epoch+1:>2}] Train Loss: {train_losses[-1]:.4f} | Val Loss: {avg_val_loss:.4f}")

    if avg_val_loss < best_val_loss:
        best_val_loss = avg_val_loss
        patience_counter = 0
        os.makedirs("checkpoints", exist_ok=True)
        torch.save(model.state_dict(), "checkpoints/decoder_transformer_best.pth")
    else:
        patience_counter += 1
...
# === Save final model
torch.save(model.state_dict(), "checkpoints/decoder_transformer_last.pth")
...
plt.savefig("figures/fig5a_decoder_loss.png", dpi=300)
plt.savefig("figures/fig5a_decoder_loss.svg")
plt.close()
