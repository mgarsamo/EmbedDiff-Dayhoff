...
# scripts/train_transformer_dayhoff.py
import sys, os
...
# === Load dataset ===
data = torch.load("data/decoder_dataset_dayhoff.pt")
token_embeddings = data["token_embeddings"]    # (B, L, 1280)
sequences = data["sequences"]                  # (B, L)
dayhoff_logits = data["dayhoff_logits"]        # (B, L, V)

# === Dataset split (80/10/10) ===
dataset = TensorDataset(token_embeddings, sequences, dayhoff_logits)
...
for epoch in range(EPOCHS):
    ...
    print(f"[Dayhoff][Epoch {epoch+1:>2}] Train Loss: {train_losses[-1]:.4f} | Val Loss: {avg_val_loss:.4f}")

    if avg_val_loss < best_val_loss:
        best_val_loss = avg_val_loss
        patience_counter = 0
        os.makedirs("checkpoints", exist_ok=True)
        torch.save(model.state_dict(), "checkpoints/decoder_transformer_dayhoff_best.pth")
    else:
        patience_counter += 1
...
# === Save final model
torch.save(model.state_dict(), "checkpoints/decoder_transformer_dayhoff_last.pth")
print("✅ Final Dayhoff decoder model saved!")

# === Plot loss curves
font_props = FontProperties(weight='bold', family='Arial', size=12)
plt.figure(figsize=(8, 5))
# Add Dayhoff label with alpha=0.0 to avoid showing dash
...
plt.savefig("figures/fig5a_decoder_loss_dayhoff.png", dpi=300)
plt.savefig("figures/fig5a_decoder_loss_dayhoff.svg")
plt.close()
